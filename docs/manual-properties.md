...
| Property | Default | What It Controls | When to Change | Impact |
|----------|---------|------------------|----------------|--------|
| `claude.binary` | `claude` | Path to the Claude CLI executable. Can be an absolute path (e.g., `/usr/local/bin/claude`) or just the binary name if it is on the system PATH. | Change when Claude CLI is installed in a non-standard location or when using a wrapper script. | If the binary is not found, all Claude features are disabled and the UI shows a "Claude not available" warning. |
...
| `claude.analysis.parallel-chunks` | `5` | Number of Claude CLI processes that execute concurrently for **enrichment** (branch chunks within a single endpoint). | Increase (e.g., `10`) on machines with many cores and sufficient API quota. Decrease (e.g., `2`) if hitting rate limits or on resource-constrained machines. | Higher values speed up enrichment linearly but consume more CPU, memory, and API quota. Each process is a separate Claude CLI invocation. |
| `claude.analysis.parallel-endpoints` | `3` | Number of endpoints processed **in parallel** during the correction phase (`ClaudeCorrectionService`). Separate from `parallel-chunks`, which controls within-endpoint parallelism. | Increase to speed up full-scan corrections on machines with spare cores and API quota. Decrease to reduce concurrent database and API load. | Each parallel endpoint spawns its own Claude process with a dedicated decompiled-source context. |
